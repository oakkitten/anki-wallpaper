...
def get_addon_folder_path() -> str:
    this_file_folder, _file_name = os.path.split(__file__)
    return os.path.join(this_file_folder, "../anki_wallpaper")
    ...
    with waitress_patched_to_prevent_it_from_dying():
        with anki_patched_to_prevent_backups():
            with anki_running(
                ...
                unpacked_addons=[("anki_wallpaper", get_addon_folder_path())]
            ) as session:
                yield session
